...
# --- 1. ROBUST ASSET LOADER (Fixes FileNotFoundError) ---
def load_asset(filename):
    # Get the absolute path of the directory containing THIS script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # Build the full path to the file inside the 'assets' folder
    ...
    try:
        ...
        st.error(f"❌ Critical Error: Could not find '{filename}' at: {file_path}")
        st.stop()
        return ""

# --- 2. CONFIGURATION ---
# Setup data directory next to the script
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(SCRIPT_DIR, "agnos_data")
...
def load_org_data(org_key):
    ...
    # Cache Today's Logs (Fast CSV Read)
...
def save_log(name, date_str, time_str):
    ...
    # Instant RAM check
    if name in st.session_state.logged_set: return False 
...
if not st.session_state.auth_status:
    ...
    with col2:
        ...
        st.markdown("Enter Organization Key")
        ...
        if len(key_in) == 5:
            ...
            else: st.warning(f"🆕 Creating New Org: {key_in}")

        with st.form("auth"):
            btn_txt = "Sign In" if is_known else "Create Account"
            pw_in = st.text_input("Password", type="password")
            if st.form_submit_button(btn_txt, type="primary"):
                if len(pw_in) < 1: st.error("Password required")
                else:
                    h_pw = hashlib.sha256(pw_in.encode()).hexdigest()
                    if is_known:
    ...
    with tab1:
        ...
        with c1: 
            # Use component_key to force full reset on logout
